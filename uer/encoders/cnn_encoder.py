...
class GatedcnnEncoder(nn.Layer):
    def __init__(self, args):
        ...
        self.gate = nn.LayerList([nn.Conv2D(args.hidden_size, args.hidden_size, (args.kernel_size, 1)) \
            for _ in range(args.layers_num-1)])
    ...
    def forward(self, emb, seg):
        ...
        padding = paddle.zeros([batch_size, self.kernel_size-1, self.emb_size])
        emb = paddle.concat([padding, emb], dim=1).unsqueeze(1) # batch_size, 1, seq_length+width-1, emb_size

        hidden = self.conv_1(emb)
        gate = self.gate_1(emb)
        hidden = hidden * paddle.sigmoid(gate)
        ...
        for i, (conv_i, gate_i) in enumerate(zip(self.conv, self.gate)):
            ...
            if (i + 1) % self.block_size:
                ...
                res_input = hidden
            hidden = paddle.concat([padding, hidden], dim=2)
