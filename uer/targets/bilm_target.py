...
class BilmTarget(nn.Layer):
    ...
    def forward(self, memory_bank, tgt):
        ...
        one_hot_forward = paddle.zeros([label_mask_forward.size(0),  self.vocab_size]).scatter_(1, tgt_forward, 1.0)
        numerator_forward = -paddle.sum(output_forward * one_hot_forward, 1)
        label_mask_forward = label_mask_forward.contiguous().view(-1)
        tgt_forward = tgt_forward.contiguous().view(-1)
        numerator_forward = paddle.sum(label_mask_forward * numerator_forward)
        denominator_forward = paddle.sum(label_mask_forward) + 1e-6
        loss_forward = numerator_forward / denominator_forward
        correct_forward = paddle.sum(label_mask_forward * (output_forward.argmax(dim=-1).eq(tgt_forward)).float())
        ...
        one_hot_backward = paddle.zeros([label_mask_backward.size(0),  self.vocab_size]).scatter_(1, tgt_backward, 1.0)
        numerator_backward = -paddle.sum(output_backward * one_hot_backward, 1)
        label_mask_backward = label_mask_backward.contiguous().view(-1)
        tgt_backward = tgt_backward.contiguous().view(-1)
        numerator_backward = paddle.sum(label_mask_backward * numerator_backward)
        denominator_backward = paddle.sum(label_mask_backward) + 1e-6
        loss_backward = numerator_backward / denominator_backward
        correct_backward = paddle.sum(label_mask_backward * (output_backward.argmax(dim=-1).eq(tgt_backward)).float())
