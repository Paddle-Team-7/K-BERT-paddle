...
class BertTarget(nn.Layer):
    ...
    def mlm(self, memory_bank, tgt_mlm):
        ...
        one_hot = paddle.zeros([output_mlm.size(0),  self.vocab_size]).scatter_(1, tgt_mlm.contiguous().view(-1,1), 1.0)
        numerator = -paddle.sum(output_mlm * one_hot, 1)
        denominator = paddle.tensor(output_mlm.size(0) + 1e-6)
        loss_mlm = paddle.sum(numerator) / denominator
        if output_mlm.size(0) == 0:
            correct_mlm = paddle.to_tensor(0.0)
        else:
            correct_mlm = paddle.sum((output_mlm.argmax(dim=-1).eq(tgt_mlm)).float())
    ...
    def forward(self, memory_bank, tgt):
        ...
        output_nsp = paddle.tanh(self.nsp_linear_1(memory_bank[:, 0, :]))
