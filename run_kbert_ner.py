...
class BertTagger(nn.Layer):
    ...
    def forward(self, src, label, mask, pos=None, vm=None):
        ...
        one_hot = paddle.zeros([label_mask.size(0),  self.labels_num]).scatter_(1, label, 1.0)
...
def main():
    ...
    def evaluate(args, is_test):
        ...
        input_ids = paddle.to_tensor([sample[0] for sample in dataset],dtype='int64')
        label_ids = paddle.to_tensor([sample[1] for sample in dataset],dtype='int64')
        mask_ids = paddle.to_tensor([sample[2] for sample in dataset],dtype='int64')
        pos_ids = paddle.to_tensor([sample[3] for sample in dataset],dtype='int64')
        vm_ids = paddle.to_tensor([sample[4] for sample in dataset],dtype='int64')
        tag_ids = paddle.to_tensor([sample[5] for sample in dataset],dtype='int64')
        ...
        confusion = paddle.zeros([len(labels_map), len(labels_map)], dtype='int64')
    ...
    input_ids = paddle.to_tensor([ins[0] for ins in instances],dtype='int64')
    label_ids = paddle.to_tensor([ins[1] for ins in instances],dtype='int64')
    mask_ids = paddle.to_tensor([ins[2] for ins in instances],dtype='int64')
    pos_ids = paddle.to_tensor([ins[3] for ins in instances],dtype='int64')
    vm_ids = paddle.to_tensor([ins[4] for ins in instances],dtype='int64')
    tag_ids = paddle.to_tensor([ins[5] for ins in instances],dtype='int64')
