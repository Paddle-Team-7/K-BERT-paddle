...
class BertClassifier(nn.Layer):
    ...
    def forward(self, src, label, mask, pos=None, vm=None):
        ...
        if self.pooling == "mean":
            ...
            output = paddle.max(output, axis=1)[0]
...
def main():
    ...
    def evaluate(args, is_test, metrics='Acc'):
        ...
        pos_ids = paddle.to_tensor([example[3] for example in dataset],dtype='int64')
        ...
        correct = 0
        # Confusion matrix.
        confusion = paddle.zeros([args.labels_num, args.labels_num], dtype='int64')
        ...
        if not args.mean_reciprocal_rank:
            ...
            for i, (input_ids_batch, label_ids_batch,  mask_ids_batch, pos_ids_batch, vms_batch) in enumerate(batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms)):
                ...
                if i == 0:
                    logits_all=logits
                if i >= 1:
                    logits_all=paddle.concat((logits_all,logits),0)
    ...
    print("input_ids")
    input_ids = paddle.to_tensor([example[0] for example in trainset],dtype='int64')
    print("label_ids")
    label_ids = paddle.to_tensor([example[1] for example in trainset],dtype='int64')
    print("mask_ids")
    mask_ids = paddle.to_tensor([example[2] for example in trainset],dtype='int64')
    print("pos_ids")
    pos_ids = paddle.to_tensor([example[3] for example in trainset],dtype='int64')
    print("vms")
